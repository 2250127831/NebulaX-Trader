...
> 日期：2026-08-04 · 分支：V1 · 工具：perf（CPU 热点 + 硬件事件）+ LensX（eBPF 延迟）+ 压测（吞吐/丢包）
>
> 一句话结论：**V1 吞吐临界 ≈5.55M msg/s（book_th 订单簿 std::map 红黑树是瓶颈，IPC 0.55 内存受限）；端到端延迟 P50 ≈20µs / P99 ≈3.1ms，长尾全在 SPMC 排队（push_spmc→pop，被最慢消费者 book_th 拖累）；各段计算路径全部 ≤7µs。V2 主攻订单簿缓存友好化（替换 std::map 价格档，吞吐+延迟同源）。**
>
> **测试方法见 [V1_TEST_HANDBOOK.md](V1_TEST_HANDBOOK.md)**（三类测量脚本用法 + 外部依赖 + 复现流程）。
...
- **限速**：`--rate N` = 每 0.1s 发 N 包，**平滑铺开**（每包 `sleep(100ms/N)`，瞬时≈平均）。瞬时=平均才测得出真实临界。**QPS ≈ 505N msg/s**（每包平均 50.5 条，见 4.1）。
- **零丢判定**：`发送数 == 解析总数`。内部通道有背压（见 0.2）不丢，丢只可能在 UDP 层。
- **保证方式**：纯 UDP 固定速率无法数学保证零丢（发送端不感知接收端消费速度）。达成方式是**留足余量 + 多次验证**：
  - **标准零丢档：`--rate 10000`（~5.05M msg/s）**（方案A单通道下 book_th 处理全部事件，临界降低，留余量）。
...
**QPS 换算**：`--rate N` = 每 0.1s 发 N 包（平滑限速）= **10N 包/s**；每包随机 1~`pack_max`（默认 100）条消息，平均 **50.5 条/包**（`benchmark/main.cpp`）。故 **QPS ≈ 10N × 50.5 ≈ 505N msg/s**。

| --rate | 包/s | **QPS(msg/s)** | drops_b |
|---|---|---:|---|
| 8000 | 80K | **~4.04M** | **0** ✅ |
| 10000 | 100K | **~5.05M** | **0** ✅ |
| 11000 | 110K | **~5.55M** | **0** ✅（临界） |
| 12000 | 120K | **~6.06M** | **UDP 丢** ❌ |

**结论：方案A下 book_th 是吞吐瓶颈，临界 ≈ 5.55M msg/s（11000 零丢 / 12000 丢）。** 超过它内部通道背压把压力传导回 UDP 层，入口丢（内部零丢）。瓶颈归因：book_th 处理全部事件（成交更新挂单 + 委托重建盘口），订单簿计算是热点。V2 订单簿缓存友好化见 6.1。
...
延迟分布（`--rate 10000` 零丢档，CSV 原始数据离线分析，剔除 `>10¹²ns` 异常值）：

| 段 | n | P50 | P99 | P999 | 语义 |
|---|---|---:|---:|---:|---|
| 包级 recv→unpack | 346K | 1.7µs | 2.2µs | 6.1µs | recv_th 内部 |
| alloc→push_ring | 134K | 1.5µs | 2.1µs | 22.3µs | 构造+入字节 ring(recv_th) |
| **push_ring→parse** | 131K | **0.2µs** | 5.7µs | 70.5µs | **SPSC 跨线程(recv→parse), 近零** |
| parse→push_spmc | 132K | 1.6µs | 2.6µs | 9.4µs | 解析→入 SPMC(parse_th) |
| **push_spmc→pop** | 132K | **5.8µs** | **3070µs** | **4173µs** | **SPMC 排队(parse→book), 长尾全在这** |
| **alloc→pop(总)** | 66K | 10.2µs | 3075µs | 4176µs | 一条消息从分配到被消费 |
| 仲裁 arb→end | 133K | 1.7µs | 2.7µs | 4.8µs | 仲裁完整执行, 非瓶颈 |
| 下单 order→end | 15 | 6.7µs | 10.7µs | 10.7µs | 下单决策→send, 快 |

> **测量方法**：数据来自 `--rate 10000` 零丢档一次的 **CSV 原始数据离线分析**（`scripts/measure_lensx.sh` 末尾的 Python 分析段）。不依赖 LensX 终端 Results（其 dump 样本量随 SIGINT 时机波动，不可靠）。剔除 `>10¹²ns` 异常值（key 模式跨线程 ts 序差导致的假样本）。**各段子母关系自洽**：子段 P50/P99/P999 均 ≤ 总段（如 push_spmc→pop 5.8µs ≤ alloc→pop 10.2µs）。

### 全链路延迟累加约算

一条消息从收包到下单的完整旅程（串联各段 P50/P99）：

| 段 | P50 | P99 | P999 |
|---|---:|---:|---:|
| recv→unpack（包级） | 1.7µs | 2.2µs | 6.1µs |
| alloc→pop（消息级，已含分配→消费） | 10.2µs | 3075µs | 4176µs |
| 仲裁 arb→end | 1.7µs | 2.7µs | 4.8µs |
| 下单 order→end | 6.7µs | 10.7µs | 10.7µs |
| **全链路累加** | **≈20µs** | **≈3.1ms** | **≈4.2ms** |

**直观结论**：
- **P50 ≈ 20µs**：正常路径收包→下单发出约 20 微秒。
- **P99 ≈ 3.1ms / P999 ≈ 4.2ms**：长尾由 alloc→pop 的 SPMC 排队（P99 3075µs / P999 4176µs）主导，占全链路 ~99%；其余各段合计 <16µs。
- **一句话**：全链路延迟 ≈ **SPMC 排队延迟 + 固定开销**。SPMC 排队是唯一可变大头（P50 5.8µs → P999 4.2ms），其余（拆包/解析/仲裁/下单）都是微秒级固定成本。

### 延迟瓶颈结论
1. **各段计算路径全部 ≤7µs，无单段计算瓶颈**：拆包 1.7µs、解析 1.6µs、仲裁 1.7µs、下单 6.7µs——计算全程极快。
2. **端到端延迟大头 = push_spmc→pop（P50 5.8µs / P99 3070µs）= SPMC 排队**：alloc→pop 拆 4 段后，前 3 段（含 SPSC 跨线程）全部干净（≤1.6µs），**唯一热点是 SPMC→book_th 排队**。book_th 处理全部事件（委托占 ~97%），消费慢 → SPMC 积压。
3. **SPSC vs SPMC 跨线程对比**：SPSC 字节 ring 跨线程 P50 0.2µs（近零）vs SPMC 排队 P50 5.8µs / P99 3070µs——**相差 1-3 个数量级**。SPSC 单消费者（parse_th 快），永不积压；SPMC 多消费者共享，**被最慢消费者（book_th）拖累**。这是 SPMC 多消费者广播的固有代价，非队列实现问题。
4. **长尾根源 = book_th 消费速度（吞吐瓶颈）**：队列满 → 等最慢消费者 → 长尾。与吞吐瓶颈同源（book_th 临界 ~5.5M，压测 5M 接近临界）。
5. **SPMC 优化（2026-08-04）已生效**：lazy progress + cache padding + batch reclaim + **cached min_head**（满时才扫描）。cached min_head 消除每次 push 的 O(consumers) 原子遍历，减少"假满"。
