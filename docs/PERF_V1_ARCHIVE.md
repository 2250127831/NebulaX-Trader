...
| 段 | P50 | P99 |
|---|---:|---:|
| recv→unpack（包级） | 1.7µs | 2.2µs |
| alloc→pop（消息级，已含分配→消费） | 10.2µs | 3075µs |
| 仲裁 arb→end | 1.7µs | 2.7µs |
| 下单 order→end | 6.7µs | 10.7µs |
| **全链路累加** | **≈20µs** | **≈3.1ms** |

**直观结论**：
- **P50 ≈ 20µs**：正常路径收包→下单发出约 20 微秒。
- **P99 ≈ 3.1ms**：长尾由 alloc→pop 的 SPMC 排队（P99 3075µs）主导，占全链路 ~99%；其余各段合计 <16µs。
- **一句话**：全链路延迟 ≈ **SPMC 排队延迟 + 固定开销**。SPMC 排队是唯一可变大头（P50 5.8µs → P99 3ms），其余（拆包/解析/仲裁/下单）都是微秒级固定成本。
