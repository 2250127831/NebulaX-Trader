...
一条消息从收包到下单的完整旅程（串联各段 P50/P90/P99）：

| 段 | P50 | P90 | P99 | 说明 |
|---|---:|---:|---:|---|
| recv→unpack（包级） | 1µs | 3µs | 3µs | 收包→拆包 |
| alloc→pop（消息级，已含分配→消费） | 24µs | 196µs | 786µs | 含 SPMC 排队 |
| 仲裁 arb→end | 1µs | 3µs | 3µs | 仲裁判定 |
| 下单 order→end | 6µs | 6µs | 6µs | 决策→send |
| **全链路累加** | **≈32µs** | **≈208µs** | **≈798µs** | 收包→下单 send |

**直观结论**：
- **P50 ≈ 32µs**：一条消息正常路径下，收包到下单发出约 32 微秒（毫秒级以下）。
- **P90 ≈ 208µs / P99 ≈ 798µs**：长尾由 **alloc→pop 的 SPMC 排队（196µs/786µs）** 主导——占全链路 90%+。其余各段合计 <12µs。
- **一句话**：全链路延迟 ≈ **SPMC 排队延迟 + 12µs 固定开销**。SPMC 排队是唯一可变大头，其余（拆包/解析/仲裁/下单）都是微秒级固定成本。
