...
void OrderBookConsumer::handle_add(const MarketEvent& ev) {
    book_for(ev.locate).add(ev.order.order_ref, ev.order.side,
                            ev.order.price, ev.order.shares, ++order_seq_);
}

void OrderBookConsumer::handle_delete(const MarketEvent& ev) {
    book_for(ev.locate).remove(ev.order.order_ref);
}

void OrderBookConsumer::handle_cancel(const MarketEvent& ev) {
    book_for(ev.locate).cancel(ev.order.order_ref, ev.order.shares);
}

void OrderBookConsumer::handle_replace(const MarketEvent& ev) {
    // U 无方向：继承旧单方向（旧单不存在则无法重建，跳过）
    OrderBook::OrderSlotView old;
    if (!book_for(ev.locate).get(ev.order.order_ref, old)) return;
    book_for(ev.locate).replace(ev.order.order_ref, ev.order.new_order_ref,
                                old.side, ev.order.price, ev.order.shares, ++order_seq_);
}

void OrderBookConsumer::handle_trade(const MarketEvent& ev, bool has_price) {
    book_for(ev.locate).execute(ev.trade.order_ref, ev.trade.volume);
...
void OrderBookConsumer::handle_execute(const MarketEvent& ev) {
    // E 不带价格：从簿查该挂单的价格
    OrderBook::OrderSlotView view;
    int64_t price = -1;
    if (book_for(ev.locate).get(ev.trade.order_ref, view)) price = view.price;
    book_for(ev.locate).execute(ev.trade.order_ref, ev.trade.volume);
